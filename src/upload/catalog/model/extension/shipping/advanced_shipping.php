    ...
    function getQuote($address)
    {
        ...
        if ($shippings) {
            foreach ($shippings as $key => $value) {
                ...
                if ($status && $price_status && $weight_status && $customer_status && $quantity_status) {
                    
                    $tax_class_id = isset($value['tax_class_id']) ? $value['tax_class_id'] : 0;
                    
                    if ($this->config->get('shipping_advanced_shipping_replace_free') && $value['cost'] == 0) {
        ...
        $module_title = $this->config->get('shipping_advanced_shipping_name');
